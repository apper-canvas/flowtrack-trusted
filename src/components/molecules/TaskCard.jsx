...
const TaskCard = ({ task, onUpdate, onDelete }) => {
  ...
  const [editTitle, setEditTitle] = useState(task.title_c || task.title || "")
  const [editDescription, setEditDescription] = useState(task.description_c || task.description || "")

  const handleToggleComplete = () => {
    const currentStatus = task.status_c || task.status;
    onUpdate(task.Id, {
      status_c: currentStatus === "completed" ? "active" : "completed",
      completed_at_c: currentStatus === "completed" ? null : new Date().toISOString()
    })
  }
...
const handleCancelEdit = () => {
    setEditTitle(task.title_c || task.title || "")
    setEditDescription(task.description_c || task.description || "")
    setIsEditing(false)
  }
...
const isCompleted = (task.status_c || task.status) === "completed"
...
<div className="text-xs text-slate-500 space-y-1">
          <div>Created {format(new Date(task.created_at_c || task.createdAt || task.CreatedOn), "MMM dd, yyyy")}</div>
          {isCompleted && (task.completed_at_c || task.completedAt) && (
            <div className="text-success-600">
              Completed {format(new Date(task.completed_at_c || task.completedAt), "MMM dd, yyyy")}
            </div>
          )}
        </div>
